...
// -----------------------
fn godot_value_to_variant(value: GodotValue) -> Result<Variant> {
    match value {
...
// -----------------------
#[allow(dead_code)]
pub fn import_top_level_resource(value: GodotValue, frontmatter: HashMap<String, GodotValue>, save_path: Option<String>) -> Result<Gd<Resource>> {
    if !matches!(value, GodotValue::Resource { type_name: _, fields: _ }){
...
fn instantiate_resource(type_name: &str) -> Result<Gd<Resource>> {
    ...
    if ClassDb::singleton().class_exists(&StringName::from(type_name)) {
        let inst = ClassDb::singleton()
            .instantiate(&StringName::from(type_name));
...
// -----------------------
pub fn build_top_level_resource(value: GodotValue, path : Option<String>, frontmatter: &HashMap<String, GodotValue>) -> Result<Gd<Resource>> {
    let res = match value {
        GodotValue::Resource { type_name, fields : _ } => {
...
// -----------------------
fn apply_doke_frontmatter_if_exists(resource: &mut Gd<Resource>, frontmatter: &HashMap<String, GodotValue>)-> Result<()> {
    resource.call(APPLY_ROOT_DOKE_FM_METHOD, &[convert_fm_to_godot(frontmatter)?]);
    Ok(())
}
